import { IGraphBuilder, IGraphResult } from '../../Domain/Services/IGraphBuilder';
...
export class GraphBuilder implements IGraphBuilder {
    ...
    build(files: SourceFile[], entryPointIds: string[]): IGraphResult {
        ...
        const rawMap = new Map<string, SourceFile>();
        ...
        const processFrom = (startId: string) => {
            ...
            while (queue.length > 0) {
                ...
                (raw.dependencies || []).forEach((dep: string) => {
    ...
    private resolveEventBridge() {
        ...
        const normalize = (name: string) => name.toLowerCase().replace(/^on/, '');
    ...
    private processOrphanTree(startId: string, parentId: string, rawMap: Map<string, SourceFile>, visited: Set<string>) {
        if (visited.has(startId)) return;
        
        const queue: { id: string, pid: string }[] = [{ id: startId, pid: parentId }];
    ...
    private resolveInterstellar(rawMap: Map<string, SourceFile>) {
