...
export class PolarLayoutStrategy implements ILayoutStrategy {
    ...
    applyLayout(nodes: Record<string, GraphNode>): void {
        ...
        const distribute = (parentId: string, parentAngle: number, parentWedge: number) => {
    ...
    private distributeChildren(parentId: string, parentAngle: number, parentWedge: number, childrenMap: Map<string, GraphNode[]>) {
        const children = childrenMap.get(parentId) || [];
        if (children.length === 0) return;
